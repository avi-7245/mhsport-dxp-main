...
@Component(immediate = true, property = {
		...
		"mvc.command.name="
				+ CompetitionManagementWebPortletKeys.DOWNLOAD_RESULT_MVC_RESOURCE_COMMAND }, service = MVCResourceCommand.class)

public class DownloadResultMVCResourceCommand implements MVCResourceCommand {

	private static final Log LOGGER = LogFactoryUtil.getLog(DownloadResultMVCResourceCommand.class);
	...
	public boolean serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		long resultUploadId = ParamUtil.getLong(resourceRequest, "resultUploadId");
		...
		try {
			...
			CompetitionResultUpload resultUpload = CompetitionResultUploadLocalServiceUtil
					.getCompetitionResultUpload(resultUploadId);
	...
	private ByteArrayOutputStream generateWinnerCertificatePDF(String winnerName, String competitionName,
		...
		try (PDPageContentStream contentStream = new PDPageContentStream(document, page)) {
			...
			// Background
			contentStream.setNonStrokingColor(new Color(245, 245, 245)); // light grey
			contentStream.addRect(0, 0, page.getMediaBox().getWidth(), page.getMediaBox().getHeight());
			contentStream.fill();

			// Add Logo (Site or portal logo)
			try {
			...
			drawCenteredText(contentStream, "Competition Result Summary", centerX, startY - 100, -50);

			contentStream.setFont(PDType1Font.HELVETICA_BOLD, 16);
			drawCenteredText(contentStream, "Competition: " + competitionName, centerX, startY - 150);
			drawCenteredText(contentStream, "Organized by: " + organizerName, centerX, startY - 180);

			contentStream.setFont(PDType1Font.HELVETICA, 14);
			drawCenteredText(contentStream, "This document contains the final results for this competition.", centerX,
					startY - 210);

			// Draw Results Table
			float tableStartY = startY - 270;
			float rowHeight = 25;
			float tableStartX = centerX - 150;
			float tableWidth = 300;

			String[][] resultData = { { "Position", "Participant Name" }, { "Winner", winnerName },
					{ "First Runner-Up", "First Runner Up Name" }, { "Second Runner-Up", "Second Runner Up Name" },
					{ "Third Runner-Up", "Third Runner Up Name" } };

			for (int i = 0; i < resultData.length; i++) {
				float currentY = tableStartY - (i * rowHeight);
				contentStream.setStrokingColor(Color.DARK_GRAY);
				contentStream.addRect(tableStartX, currentY - rowHeight, tableWidth, rowHeight);
				contentStream.stroke();

				contentStream.beginText();
				contentStream.setFont(PDType1Font.HELVETICA_BOLD, 12);
				contentStream.newLineAtOffset(tableStartX + 10, currentY - 17);
				contentStream.showText(resultData[i][0]);
				contentStream.endText();

				contentStream.beginText();
				contentStream.setFont(PDType1Font.HELVETICA, 12);
				contentStream.newLineAtOffset(tableStartX + 150, currentY - 17);
				contentStream.showText(resultData[i][1]);
				contentStream.endText();
			}

			// Footer
			contentStream.setFont(PDType1Font.HELVETICA, 12);
			drawCenteredText(contentStream, "Date: " + formatter.format(new Date()), centerX - 100, tableStartY - 180);
			drawCenteredText(contentStream, "_________________________", centerX + 100, tableStartY - 190);
			drawCenteredText(contentStream, "Authorized Signatory", centerX + 100, tableStartY - 210);
			...
			String fileName = "Competition_Result_" + timestamp + ".pdf";

			File tempFile = File.createTempFile("competition_result_", ".pdf");
			...
			LOGGER.error("Error while generating competition result PDF", e);
		} finally {
			document.close();
		}
