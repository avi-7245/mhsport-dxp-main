...
#establishment-list_wrapper .top {
        display: flex;
        justify-content: space-between;
}
#establishment-list_wrapper .bottom {
   /* display: flex;
    justify-content: space-between;*/
			...
			<div class="col-md-12">
				<div class="border-0 card shadow">
					...
					<div class="align-items-center back-btn-cn card-header d-flex justify-content-between" style="">
						<liferay-ui:message key="establishment-list"/>
					...
					<!-- filter -->
						<div class="filter-container">
							...
							<div class="filter-card mt-4" id="filterCard">
								...
								<form id="establishment" method="POST">
									<div class="card-body">
										<div class="row">
										    
											<div class="col-md-6">
	                                            <div class="form-group">
	                                                <label><liferay-ui:message key="employee-name"/></label>
					...
					<div class="universal-table mt-4">
						<div class="">
								<table id="establishment-list" class="table-bordered" cellspacing="0" width="100%">
									<thead>
										<tr>
											<th><liferay-ui:message key="employee-name" /></th>
											<th><liferay-ui:message key="sevarath-id" /></th>
											<th><liferay-ui:message key="education" /></th>
											<th><liferay-ui:message key="hometown" /></th>
											<th><liferay-ui:message key="action" /></th>
										</tr>
									</thead>
									<tbody>
										<c:forEach var="details" items="${detailses}">
											<tr>
												<td>${details.employeeName}</td>
												<td>${details.sevarathId}</td>
												...
												<portlet:renderURL var="editDetailsURL">
													...
													<portlet:param name="userId" value="${details.userId}"></portlet:param>
													<portlet:param name="cmd" value="edit"></portlet:param>
												</portlet:renderURL>
												
												<td>
													<a href="${editDetailsURL}" class="btn btn-primary"><i class="bi bi-eye icons-color" title="<liferay-ui:message key="view-edit" />"></i></a>
												</td>
...
var table = $('#establishment-list').DataTable({
    "paging": true,
    "ordering": false,
    "searching": true,
    "responsive": true,
...
$(document).on('click', '#searchBtn', function () {
    ...
    if (employeeName) table.column(0).search(employeeName);
    if (role) table.column(1).search(role);
